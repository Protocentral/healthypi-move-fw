...
void draw_scr_bpt_cal_complete(enum scroll_dir m_scroll_dir, uint32_t arg1, uint32_t arg2, uint32_t arg3, uint32_t arg4)
{
    ...
    lv_obj_add_style(scr_bpt_cal_complete, &style_scr_black, 0);
    //lv_obj_set_scrollbar_mode(scr_bpt_cal_complete, LV_SCROLLBAR_MODE_ON);

    /*Create a container with COLUMN flex direction*/
    lv_obj_t *cont_col = lv_obj_create(scr_bpt_cal_complete);
    ...
    lv_label_set_text(label_info, "Calibration Complete");
    ...
    lv_label_set_text(label_info2, "You can now measure your BP from the same screen. Calibration data is valid for 1 month.");
