 ...
 *      DEFINES
 *********************/
#define MY_CLASS &hpi_lv_chart_class
...
static void hpi_lv_chart_event(const lv_obj_class_t *class_p, lv_event_t *e);
...
static lv_chart_tick_dsc_t *get_tick_gsc(lv_obj_t *obj, lv_chart_axis_t axis);

/**********************
 *  STATIC VARIABLES
 **********************/
const lv_obj_class_t hpi_lv_chart_class = {
    .constructor_cb = lv_chart_constructor,
    .destructor_cb = lv_chart_destructor,
    .event_cb = hpi_lv_chart_event,
...
void hpi_lv_chart_get_point_pos_by_id(lv_obj_t *obj, lv_chart_series_t *ser, uint16_t id, lv_point_t *p_out)
{
    ...
    uint32_t start_point = hpi_lv_chart_get_x_start_point(obj, ser);
...
lv_chart_series_t *hpi_lv_chart_get_series_next(const lv_obj_t *obj, const lv_chart_series_t *ser)
{
    LV_ASSERT_OBJ(obj, MY_CLASS);
...
uint32_t hpi_lv_chart_get_pressed_point(const lv_obj_t *obj)
{
    lv_chart_t *chart = (lv_chart_t *)obj;
...
static void draw_series_bar(lv_obj_t *obj, lv_draw_ctx_t *draw_ctx)
{
    ...
    col_a.y2 = obj->coords.y2 + col_dsc.radius -30;
...
static void draw_x_ticks(lv_obj_t *obj, lv_draw_ctx_t *draw_ctx, lv_chart_axis_t axis)
{
    ...
    p1.y = y_ofs;
    for (i = 0; i <= total_tick_num; i++)
    { /*one extra loop - it may not exist in the list, empty label*/
        bool major = false;
        if (i % t->minor_cnt == 0)
            major = true;
