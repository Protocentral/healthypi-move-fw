...
void draw_scr_spo2_scr2(enum scroll_dir m_scroll_dir, uint32_t arg1, uint32_t arg2, uint32_t arg3, uint32_t arg4)
{
    ...
    if (spo2_source == SPO2_SOURCE_PPG_WR)
    {
        lv_obj_t *img_spo2_wr = lv_image_create(cont_col);
        lv_image_set_src(img_spo2_wr, &img_spo2_hand);
    ...
    {
        lv_obj_t *img_spo2_fi = lv_image_create(cont_col);
        lv_image_set_src(img_spo2_fi, &img_bpt_finger_90);
