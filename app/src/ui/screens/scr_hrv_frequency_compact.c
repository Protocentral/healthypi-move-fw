...
// Static variables for HRV frequency analysis
static double lf_power_compact = 0.0f;
static double hf_power_compact = 0.0f;
static double stress_score_compact = 0.0f;
...
void draw_scr_hrv_frequency_compact(enum scroll_dir m_scroll_dir)
{
    ...
    lv_obj_set_style_pad_all(cont_metrics, 3, LV_PART_MAIN);
       ...
       // LF Power - compact horizontal

   lv_obj_t *cont_lf = lv_obj_create(cont_metrics);
    lv_obj_set_size(cont_lf, 130, 35);
    lv_obj_set_flex_flow(cont_lf, LV_FLEX_FLOW_ROW);
    lv_obj_set_flex_align(cont_lf, LV_FLEX_ALIGN_CENTER, LV_FLEX_ALIGN_CENTER, LV_FLEX_ALIGN_CENTER);
    lv_obj_add_style(cont_lf, &style_scr_black, 0);
    lv_obj_set_style_border_width(cont_lf, 1, LV_PART_MAIN);
    lv_obj_set_style_border_color(cont_lf, lv_palette_main(LV_PALETTE_RED), LV_PART_MAIN);
    lv_obj_set_style_pad_all(cont_lf, 2, LV_PART_MAIN);
    ...
    // HF Power - very compact
    lv_obj_t *cont_hf = lv_obj_create(cont_metrics);
    lv_obj_set_size(cont_hf, 130, 35);
    lv_obj_set_flex_flow(cont_hf, LV_FLEX_FLOW_ROW);
    lv_obj_set_flex_align(cont_hf, LV_FLEX_ALIGN_CENTER, LV_FLEX_ALIGN_CENTER, LV_FLEX_ALIGN_CENTER);
    lv_obj_add_style(cont_hf, &style_scr_black, 0);
    lv_obj_set_style_border_width(cont_hf, 1, LV_PART_MAIN);
    lv_obj_set_style_border_color(cont_hf, lv_palette_main(LV_PALETTE_BLUE), LV_PART_MAIN);
    lv_obj_set_style_pad_all(cont_hf, 2, LV_PART_MAIN);
    ...
    lv_obj_add_style(label_hf_power_compact, &style_white_small, 0);
     ...
     // LF/HF ratio - very compact
    lv_obj_t *cont_lf_hf = lv_obj_create(cont_metrics);
    lv_obj_set_size(cont_lf_hf, 130, 35);
    lv_obj_set_flex_flow(cont_lf_hf, LV_FLEX_FLOW_ROW);
    lv_obj_set_flex_align(cont_lf_hf, LV_FLEX_ALIGN_CENTER, LV_FLEX_ALIGN_CENTER, LV_FLEX_ALIGN_CENTER);
    lv_obj_add_style(cont_lf_hf, &style_scr_black, 0);
    lv_obj_set_style_border_width(cont_lf_hf, 1, LV_PART_MAIN);
    lv_obj_set_style_border_color(cont_lf_hf, lv_palette_main(LV_PALETTE_PURPLE), LV_PART_MAIN);
    lv_obj_set_style_pad_all(cont_lf_hf, 2, LV_PART_MAIN);

    lv_obj_t *label_lf_hf_title = lv_label_create(cont_lf_hf);
    lv_label_set_text(label_lf_hf_title, "LF/HF:");
    lv_obj_add_style(label_lf_hf_title, &style_white_small, 0);

    label_lf_hf_ratio_compact = lv_label_create(cont_lf_hf);
    lv_label_set_text(label_lf_hf_ratio_compact, "--");
    lv_obj_add_style(label_lf_hf_ratio_compact, &style_white_small, 0);
...
void hpi_hrv_frequency_compact_update_spectrum(double *rr_intervals, int num_intervals)
{
    ...
    for (int i = 0; i < num_intervals; i++) {
        ...
        variance_total += diff * diff;
       // LOG_INF("Diff[%d]: %.2f, Squared: %.2f", i, diff, diff * diff);
    }
    variance_total /= (num_intervals - 1);
...
void hpi_hrv_frequency_compact_update_display(void)
{
    ...
    // Update LF/HF ratio
    if (label_lf_hf_ratio_compact != NULL && hf_power_compact > 0) {
        //int ratio = (int)lf_power_compact / (int)hf_power_compact;
