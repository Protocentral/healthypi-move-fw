...
// Add this callback near the other static callbacks
static void btn_settings_event_cb(lv_event_t *e)
{
    ...
    if (code == LV_EVENT_CLICKED)
    {
        LOG_INF("Settings button clicked");
        draw_scr_settings(SCROLL_LEFT); // Open the settings screen with a left scroll animation
    }
}
...
void draw_scr_pulldown(enum scroll_dir m_scroll_dir)
{
    ...
    lv_obj_set_size(slider_brightness, lv_pct(100), 20);
    ...
    // Create a flex row container for shutdown and settings buttons
    lv_obj_t *cont_row_buttons = lv_obj_create(cont_col);
    lv_obj_set_size(cont_row_buttons, LV_PCT(100), LV_SIZE_CONTENT);
    lv_obj_set_flex_flow(cont_row_buttons, LV_FLEX_FLOW_ROW);
    lv_obj_set_flex_align(cont_row_buttons, LV_FLEX_ALIGN_CENTER, LV_FLEX_ALIGN_CENTER, LV_FLEX_ALIGN_CENTER);
    lv_obj_set_style_pad_right(cont_row_buttons, -1, LV_PART_SCROLLBAR);

    // Shutdown button
    lv_obj_t *btn_shutdown = lv_btn_create(cont_row_buttons);
    lv_obj_set_size(btn_shutdown, LV_PCT(50), 80);
    ...
    lv_obj_add_style(lbl_btn_shutdown, &style_white_medium, LV_STATE_DEFAULT);
    lv_obj_center(lbl_btn_shutdown);

    // Settings button
    lv_obj_t *btn_settings = lv_btn_create(cont_row_buttons);
    lv_obj_set_size(btn_settings, LV_PCT(50), 80);
    lv_obj_add_event_cb(btn_settings, btn_settings_event_cb, LV_EVENT_ALL, NULL);

    lv_obj_t *lbl_btn_settings = lv_label_create(btn_settings);
    lv_label_set_text(lbl_btn_settings, LV_SYMBOL_SETTINGS );
    lv_obj_add_style(lbl_btn_settings, &style_white_medium, LV_STATE_DEFAULT);
