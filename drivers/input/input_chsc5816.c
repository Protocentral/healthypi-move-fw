...
#define CHSC5816_READ_ADDR             0
#define CHSC5816_READ_LENGTH           5
#define CHSC5816_OUTPUT_POINTS_PRESSED 0
#define CHSC5816_OUTPUT_COL            2
#define CHSC5816_OUTPUT_ROW            4

LOG_MODULE_REGISTER(chsc5816, CONFIG_INPUT_LOG_LEVEL);

static int chsc5816_process(const struct device *dev)
{
	uint8_t output[CHSC5816_READ_LENGTH];
	uint8_t row, col;
	bool is_pressed;
	int ret;
	...
	ret = i2c_burst_read_dt(&cfg->i2c, CHSC5816_READ_ADDR, output, CHSC5816_READ_LENGTH);
	if (ret < 0) {
		LOG_ERR("Could not read data: %i", ret);
		return -ENODATA;
	}

	is_pressed = output[CHSC5816_OUTPUT_POINTS_PRESSED];
	col = output[CHSC5816_OUTPUT_COL];
	row = output[CHSC5816_OUTPUT_ROW];
